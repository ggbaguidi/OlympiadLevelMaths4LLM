...
def test_rank_answers_counts_verified():
    ...
    results = [
        AttemptResult(
            attempt=1,
            answer=10,
            stats=AttemptStats(token_count=100, python_calls=1, python_errors=0),
        ),
        AttemptResult(
            attempt=2,
            answer=10,
            stats=AttemptStats(token_count=90, python_calls=0, python_errors=0),
        ),
        AttemptResult(
            attempt=3,
            answer=11,
            stats=AttemptStats(token_count=80, python_calls=2, python_errors=1),
        ),
    ]
...
def test_rank_prefers_verified_over_more_votes():
    ...
    results = [
        AttemptResult(
            attempt=1,
            answer=1,
            stats=AttemptStats(token_count=50, python_calls=0, python_errors=0),
        ),
        AttemptResult(
            attempt=2,
            answer=1,
            stats=AttemptStats(token_count=60, python_calls=0, python_errors=0),
        ),
        AttemptResult(
            attempt=3,
            answer=1,
            stats=AttemptStats(token_count=70, python_calls=1, python_errors=1),
        ),
        AttemptResult(
            attempt=4,
            answer=2,
            stats=AttemptStats(token_count=80, python_calls=1, python_errors=0),
        ),
    ]
...
def test_rank_does_not_let_tag_diversity_overpower_votes_when_verified_ties():
    ...
    results = [
        AttemptResult(
            attempt=1,
            answer=1,
            stats=AttemptStats(token_count=50, python_calls=1, python_errors=0),
            tag="a",
        ),
        AttemptResult(
            attempt=2,
...
def test_rank_penalizes_timeout_attempts():
    ...
    results = [
        AttemptResult(
            ...
            answer=1,
            stats=AttemptStats(
                token_count=50, python_calls=1, python_errors=0, timeout_count=0
            ),
        ),
        AttemptResult(
            attempt=2,
            answer=1,
            stats=AttemptStats(
                token_count=50, python_calls=1, python_errors=0, timeout_count=1
            ),
        ),
        AttemptResult(
            attempt=3,
...
def test_rank_magnitude_aware_boosts_outliers():
    ...
    results = [
        AttemptResult(
            attempt=0, answer=14, stats=AttemptStats(python_calls=5, python_errors=1)
        ),
        AttemptResult(
            attempt=1, answer=12, stats=AttemptStats(python_calls=5, python_errors=1)
        ),
        AttemptResult(
            attempt=2, answer=11549, stats=AttemptStats(python_calls=5, python_errors=1)
        ),  # Outlier!
        AttemptResult(
            attempt=3, answer=11, stats=AttemptStats(python_calls=5, python_errors=1)
        ),
        AttemptResult(
            attempt=4, answer=15, stats=AttemptStats(python_calls=5, python_errors=0)
        ),  # Verified
        AttemptResult(
            attempt=5, answer=16, stats=AttemptStats(python_calls=5, python_errors=0)
        ),  # Verified
        AttemptResult(
            attempt=6, answer=13, stats=AttemptStats(python_calls=5, python_errors=1)
        ),
        AttemptResult(
            attempt=7, answer=5, stats=AttemptStats(python_calls=5, python_errors=1)
        ),
        AttemptResult(
            attempt=8, answer=6, stats=AttemptStats(python_calls=5, python_errors=1)
        ),
        AttemptResult(
            attempt=9, answer=3, stats=AttemptStats(python_calls=5, python_errors=1)
        ),
        AttemptResult(
            attempt=10, answer=1, stats=AttemptStats(python_calls=5, python_errors=1)
        ),
        AttemptResult(
            attempt=11, answer=4, stats=AttemptStats(python_calls=5, python_errors=1)
        ),
    ]
    ...
    assert (
        ranked_mag[0][0] == 11549
    ), f"Expected 11549 to be top, got {ranked_mag[0][0]}"

    # Without magnitude awareness, verified small answers would win
    ranked_no_mag = rank_candidates(results, magnitude_aware=False)
    assert ranked_no_mag[0][0] in (
        15,
        16,
    ), f"Expected 15 or 16, got {ranked_no_mag[0][0]}"
...
def test_rank_magnitude_aware_no_effect_when_similar_magnitudes():
    ...
    results = [
        AttemptResult(
            attempt=0, answer=10, stats=AttemptStats(python_calls=5, python_errors=1)
        ),
        AttemptResult(
            attempt=1, answer=20, stats=AttemptStats(python_calls=5, python_errors=0)
        ),  # Verified
        AttemptResult(
            attempt=2, answer=15, stats=AttemptStats(python_calls=5, python_errors=1)
        ),
        AttemptResult(
            attempt=3, answer=25, stats=AttemptStats(python_calls=5, python_errors=1)
        ),
    ]
