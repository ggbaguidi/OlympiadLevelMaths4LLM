...
class MathRetriever:
    ...
    def load(
        ...
        return cls(
            concepts, embeddings, model_name, model_path=model_path, cpu_only=cpu_only
        )
    ...
    def _load_direct_transformer(self, model_path: str, device: str):
        ...
        try:
            ...
            print(
                "[Retriever] AutoConfig failed to detect model type, falling back to BertConfig"
            )
    ...
    def retrieve(
        ...
        metadata = {
            ...
            "avg_score": (
                round(np.mean([r.score for r in results]), 3) if results else 0.0
            ),
        }
...
def benchmark_retrieval(kb_dir: str, test_queries: list[str] | None = None):
    ...
    for query in test_queries:
        ...
        for r in results[:3]:
            print(
                f"    [{r.score:.3f}] {r.concept.concept_type}: {r.concept.content[:60]}..."
            )
