...
from .prompts import ENHANCED_TOOL_INSTRUCTION, PREFERENCE_PROMPT, TIR_PROMPT_STANDARD
...
class AIMO3Config:
    ...
    constraint_discovery_prompt_fraction: float = (
        0.25  # 25% of attempts use discovery prompt
    )
    ...
    retriever_model_path: str = (
        ""  # Local path to sentence-transformer model (for offline/Kaggle)
    )
    ...
    def from_env() -> "AIMO3Config":
        ...
        strategy_pack_mode = (
            os.getenv("AIMO3_STRATEGY_PACK_MODE", AIMO3Config.strategy_pack_mode) or ""
        ).strip()
        if not strategy_pack_mode:
            strategy_pack_mode = AIMO3Config.strategy_pack_mode
        strategy_packs = (
            os.getenv("AIMO3_STRATEGY_PACKS", AIMO3Config.strategy_packs) or ""
        ).strip()
        if not strategy_packs:
            strategy_packs = AIMO3Config.strategy_packs
        shuffle_cards = os.getenv("AIMO3_SHUFFLE_CARDS", "1").strip().lower() not in {
            "0",
            "false",
            "no",
        }
        ...
        proto = os.getenv("AIMO3_PROTOCOL", "1").strip().lower() not in {
            "0",
            "false",
            "no",
        }
        ...
        lean_toolchain_enabled = os.getenv(
            "AIMO3_LEAN_TOOLCHAIN_ENABLED", "0"
        ).strip().lower() not in {"0", "false", "no"}
        lean_toolchain_dataset_dir = (
            os.getenv("AIMO3_LEAN_DATASET_DIR", "") or ""
        ).strip()
        lean_toolchain_archive_path = (
            os.getenv("AIMO3_LEAN_ARCHIVE_PATH", "") or ""
        ).strip()
        lean_toolchain_archive_name = (
            os.getenv("AIMO3_LEAN_ARCHIVE_NAME", "") or ""
        ).strip()
        lean_toolchain_work_dir = (os.getenv("AIMO3_LEAN_WORK_DIR", "") or "").strip()
        lean_toolchain_verbose = os.getenv(
            "AIMO3_LEAN_VERBOSE", "0"
        ).strip().lower() not in {"0", "false", "no"}
        ...
        easy_exit_min_verified = _env_int(
        ...
        # Phase scheduling
        code_first_phase_s = _env_float(
            "AIMO3_CODE_FIRST_PHASE_S", AIMO3Config.code_first_phase_s
        )
        ...
        python_tool_timeout_retry_enabled = os.getenv(
            "AIMO3_PYTHON_TOOL_TIMEOUT_RETRY_ENABLED", "1"
        ).strip().lower() not in {"0", "false", "no"}
        python_tool_timeout_retry_multiplier = _env_float(
            "AIMO3_PYTHON_TOOL_TIMEOUT_RETRY_MULT",
            AIMO3Config.python_tool_timeout_retry_multiplier,
        )
        ...
        python_tool_verify_marker = (
            os.getenv(
                "AIMO3_PYTHON_TOOL_VERIFY_MARKER", AIMO3Config.python_tool_verify_marker
            )
            or ""
        ).strip() or AIMO3Config.python_tool_verify_marker
        python_tool_verify_require_marker = os.getenv(
            "AIMO3_PYTHON_TOOL_VERIFY_REQUIRE_MARKER", "1"
        ).strip().lower() not in {"0", "false", "no"}

        abort_attempt_on_python_timeout = os.getenv(
            "AIMO3_ABORT_ATTEMPT_ON_PYTHON_TIMEOUT", "1"
        ).strip().lower() not in {"0", "false", "no"}
        recycle_sandbox_on_python_timeout = os.getenv(
            "AIMO3_RECYCLE_SANDBOX_ON_PYTHON_TIMEOUT", "1"
        ).strip().lower() not in {"0", "false", "no"}

        # Verification knobs
        second_stage_verify_enabled = os.getenv(
            "AIMO3_SECOND_STAGE_VERIFY_ENABLED", "1"
        ).strip().lower() not in {"0", "false", "no"}
        second_stage_top_k = _env_int(
            "AIMO3_SECOND_STAGE_TOP_K", AIMO3Config.second_stage_verify_top_k
        )
        second_stage_cap = _env_float(
            "AIMO3_SECOND_STAGE_BUDGET_CAP", AIMO3Config.second_stage_verify_budget_cap
        )
        second_stage_fraction = _env_float(
            "AIMO3_SECOND_STAGE_BUDGET_FRACTION",
            AIMO3Config.second_stage_verify_budget_fraction,
        )

        verify_reserve_fraction = _env_float(
            "AIMO3_VERIFY_RESERVE_FRACTION", AIMO3Config.verification_reserve_fraction
        )
        verify_reserve_cap = _env_float(
            "AIMO3_VERIFY_RESERVE_CAP", AIMO3Config.verification_reserve_cap
        )
        verify_reserve_min = _env_float(
            "AIMO3_VERIFY_RESERVE_MIN", AIMO3Config.verification_reserve_min
        )

        problems_total = _env_int("AIMO3_PROBLEMS_TOTAL", AIMO3Config.problems_total)

        # Adaptive budget extension
        adaptive_budget_enabled = os.getenv(
        ...
        adaptive_budget_consensus_min_answers = _env_int(
            "AIMO3_ADAPTIVE_BUDGET_CONSENSUS_MIN_ANSWERS",
            AIMO3Config.adaptive_budget_consensus_min_answers,
        )
        adaptive_budget_consensus_min_votes = _env_int(
            "AIMO3_ADAPTIVE_BUDGET_CONSENSUS_MIN_VOTES",
            AIMO3Config.adaptive_budget_consensus_min_votes,
        )
        ...
        kernel_init_workers = _env_int(
            "AIMO3_KERNEL_INIT_WORKERS", AIMO3Config.kernel_init_workers
        )
        sandbox_pool_size = _env_int(
            "AIMO3_SANDBOX_POOL_SIZE", AIMO3Config.sandbox_pool_size
        )
        sandbox_create_on_exhaustion = os.getenv(
            "AIMO3_SANDBOX_CREATE_ON_EXHAUSTION", "1"
        ).strip().lower() not in {"0", "false", "no"}
        sandbox_reset_between_attempts = os.getenv(
            "AIMO3_SANDBOX_RESET_BETWEEN_ATTEMPTS", "1"
        ).strip().lower() not in {"0", "false", "no"}

        abort_attempt_after_python_errors = _env_int(
            "AIMO3_ABORT_ATTEMPT_AFTER_PYTHON_ERRORS",
            AIMO3Config.abort_attempt_after_python_errors,
        )
        ...
        recycle_sandbox_after_python_errors = _env_int(
            "AIMO3_RECYCLE_SANDBOX_AFTER_PYTHON_ERRORS",
            AIMO3Config.recycle_sandbox_after_python_errors,
        )

        # Constraint discovery (novel approach for hard problems)
        constraint_discovery_enabled = os.getenv(
            "AIMO3_CONSTRAINT_DISCOVERY_ENABLED", "1"
        ).strip().lower() not in {"0", "false", "no"}
        constraint_discovery_prompt_fraction = _env_float(
            "AIMO3_CONSTRAINT_DISCOVERY_PROMPT_FRACTION",
            AIMO3Config.constraint_discovery_prompt_fraction,
        )
        constraint_discovery_prefix_enabled = os.getenv(
            "AIMO3_CONSTRAINT_DISCOVERY_PREFIX_ENABLED", "1"
        ).strip().lower() not in {"0", "false", "no"}

        # Working memory scratchpad (novel approach for multi-step reasoning)
        scratchpad_enabled = os.getenv(
            "AIMO3_SCRATCHPAD_ENABLED", "1"
        ).strip().lower() not in {"0", "false", "no"}
        scratchpad_prompt_fraction = _env_float(
            "AIMO3_SCRATCHPAD_PROMPT_FRACTION", AIMO3Config.scratchpad_prompt_fraction
        )
        scratchpad_reminder_enabled = os.getenv(
            "AIMO3_SCRATCHPAD_REMINDER_ENABLED", "1"
        ).strip().lower() not in {"0", "false", "no"}

        recovery_attempts_enabled = os.getenv(
            "AIMO3_RECOVERY_ATTEMPTS_ENABLED", "1"
        ).strip().lower() not in {"0", "false", "no"}
        recovery_attempts_cap = _env_int(
            "AIMO3_RECOVERY_ATTEMPTS_CAP", AIMO3Config.recovery_attempts_cap
        )
        recovery_trigger_python_errors = _env_int(
            "AIMO3_RECOVERY_TRIGGER_PYTHON_ERRORS",
            AIMO3Config.recovery_trigger_python_errors,
        )
        recovery_min_remaining_s = _env_float(
            "AIMO3_RECOVERY_MIN_REMAINING_S", AIMO3Config.recovery_min_remaining_s
        )
        recovery_mode = (
            (os.getenv("AIMO3_RECOVERY_MODE", AIMO3Config.recovery_mode) or "")
            .strip()
            .lower()
        )
        ...
        recovery_micro_tool_call_cap = _env_int(
            "AIMO3_RECOVERY_MICRO_TOOL_CALL_CAP",
            AIMO3Config.recovery_micro_tool_call_cap,
        )
        ...
        entropy_weighting_enabled = os.getenv(
            "AIMO3_ENTROPY_WEIGHTING_ENABLED", "1"
        ).strip().lower() not in {"0", "false", "no"}
        ...
        temperature_exploration = _env_float(
            "AIMO3_TEMPERATURE_EXPLORATION",
            float(AIMO3Config.temperature_exploration or temperature),
        )
        temperature_main = _env_float(
            "AIMO3_TEMPERATURE_MAIN", float(AIMO3Config.temperature_main or temperature)
        )
        temperature_code = _env_float(
            "AIMO3_TEMPERATURE_CODE", float(AIMO3Config.temperature_code or temperature)
        )
        temperature_verification = _env_float(
            "AIMO3_TEMPERATURE_VERIFICATION",
            float(AIMO3Config.temperature_verification or temperature),
        )
        temperature_formatting = _env_float(
            "AIMO3_TEMPERATURE_FORMATTING",
            float(AIMO3Config.temperature_formatting or temperature),
        )
        exploration_attempts = _env_int(
            "AIMO3_EXPLORATION_ATTEMPTS", AIMO3Config.exploration_attempts
        )

        format_recovery_enabled = os.getenv(
            "AIMO3_FORMAT_RECOVERY_ENABLED", "1"
        ).strip().lower() not in {"0", "false", "no"}
        format_recovery_cap = _env_int(
            "AIMO3_FORMAT_RECOVERY_CAP", AIMO3Config.format_recovery_cap
        )
        format_recovery_trigger_tokens = _env_int(
            "AIMO3_FORMAT_RECOVERY_TRIGGER_TOKENS",
            AIMO3Config.format_recovery_trigger_tokens,
        )
        format_recovery_min_remaining_s = _env_float(
            "AIMO3_FORMAT_RECOVERY_MIN_REMAINING_S",
            AIMO3Config.format_recovery_min_remaining_s,
        )

        # Contradiction-driven retry
        contradiction_retry_enabled = os.getenv(
            "AIMO3_CONTRADICTION_RETRY_ENABLED", "1"
        ).strip().lower() not in {"0", "false", "no"}
        contradiction_retry_max_top_votes = _env_int(
            "AIMO3_CONTRADICTION_RETRY_MAX_TOP_VOTES",
            AIMO3Config.contradiction_retry_max_top_votes,
        )
        contradiction_retry_min_distinct_answers = _env_int(
            "AIMO3_CONTRADICTION_RETRY_MIN_DISTINCT_ANSWERS",
            AIMO3Config.contradiction_retry_min_distinct_answers,
        )
        contradiction_retry_min_remaining_s = _env_float(
            "AIMO3_CONTRADICTION_RETRY_MIN_REMAINING_S",
            AIMO3Config.contradiction_retry_min_remaining_s,
        )
        contradiction_retry_budget_cap_s = _env_float(
            "AIMO3_CONTRADICTION_RETRY_BUDGET_CAP_S",
            AIMO3Config.contradiction_retry_budget_cap_s,
        )

        finalize_answer_enabled = os.getenv(
            "AIMO3_FINALIZE_ANSWER_ENABLED", "1"
        ).strip().lower() not in {"0", "false", "no"}
        ...
        finalize_answer_min_remaining_s = _env_float(
            "AIMO3_FINALIZE_ANSWER_MIN_REMAINING_S",
            AIMO3Config.finalize_answer_min_remaining_s,
        )

        # Conclusion nudge (intra-attempt prompting to force synthesis)
        conclude_nudge_enabled = os.getenv(
            "AIMO3_CONCLUDE_NUDGE_ENABLED", "1"
        ).strip().lower() not in {"0", "false", "no"}
        conclude_nudge_tokens = _env_int(
            "AIMO3_CONCLUDE_NUDGE_TOKENS", AIMO3Config.conclude_nudge_tokens
        )
        conclude_nudge_once = os.getenv(
            "AIMO3_CONCLUDE_NUDGE_ONCE", "1"
        ).strip().lower() not in {"0", "false", "no"}
        recent_python_outputs_in_conclusion_enabled = os.getenv(
            "AIMO3_RECENT_PYTHON_OUTPUTS_IN_CONCLUSION_ENABLED", "1"
        ).strip().lower() not in {"0", "false", "no"}
        recent_python_outputs_in_conclusion_n = _env_int(
            "AIMO3_RECENT_PYTHON_OUTPUTS_IN_CONCLUSION_N",
            AIMO3Config.recent_python_outputs_in_conclusion_n,
        )
        ...
        tiebreak_budget_cap_s = _env_float(
            "AIMO3_TIEBREAK_BUDGET_CAP_S", AIMO3Config.tiebreak_budget_cap_s
        )
        ...
        adversarial_debate_enabled = os.getenv(
            "AIMO3_ADVERSARIAL_DEBATE_ENABLED", "1"
        ).strip().lower() not in {"0", "false", "no"}
        adversarial_debate_min_remaining_s = _env_float(
            "AIMO3_ADVERSARIAL_DEBATE_MIN_REMAINING_S",
            AIMO3Config.adversarial_debate_min_remaining_s,
        )
        adversarial_debate_budget_cap_s = _env_float(
            "AIMO3_ADVERSARIAL_DEBATE_BUDGET_CAP_S",
            AIMO3Config.adversarial_debate_budget_cap_s,
        )
        adversarial_debate_rounds = _env_int(
            "AIMO3_ADVERSARIAL_DEBATE_ROUNDS", AIMO3Config.adversarial_debate_rounds
        )
        adversarial_debate_use_arbiter = os.getenv(
            "AIMO3_ADVERSARIAL_DEBATE_USE_ARBITER", "1"
        ).strip().lower() not in {"0", "false", "no"}

        second_stage_verify_marker = (
            os.getenv(
                "AIMO3_SECOND_STAGE_MARKER", AIMO3Config.second_stage_verify_marker
            ).strip()
            or AIMO3Config.second_stage_verify_marker
        )
        second_stage_verify_require_marker = os.getenv(
            "AIMO3_SECOND_STAGE_REQUIRE_MARKER", "1"
        ).strip().lower() not in {"0", "false", "no"}
