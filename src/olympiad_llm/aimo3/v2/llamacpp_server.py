"""llama.cpp OpenAI-compatible server subprocess management for AIMO-3 solver."""
...
from .require import _require_llama_cpp_server
...
class LlamaCppServer:
    ...
    def start(self) -> None:
        if not self.cfg.model_path:
            raise ValueError(
                "AIMO3Config.model_path is empty. Set env AIMO3_MODEL_PATH to a GGUF file or pass it explicitly."
            )

        _require_llama_cpp_server()
        ...
        n_gpu_layers = int(getattr(self.cfg, "llama_cpp_n_gpu_layers", -1) or -1)
        if n_gpu_layers != 0:
