...
class MathConcept:
    ...
    concept_type: str  # theorem, definition, proposition, example, lemma, corollary, etc.
    title: str | None
    content: str
    source_book: str
...
CONCEPT_PATTERNS = [
    # Formal structures
    (r"(?i)^(theorem|thm\.?)\s*(\d+[\.\d]*)?[:\.\s]*(.*?)(?=\n\n|\n(?=(?:theorem|definition|lemma|corollary|proposition|proof|example|remark))|\Z)", "theorem"),
    (r"(?i)^(definition|def\.?)\s*(\d+[\.\d]*)?[:\.\s]*(.*?)(?=\n\n|\n(?=(?:theorem|definition|lemma|corollary|proposition|proof|example|remark))|\Z)", "definition"),
    (r"(?i)^(lemma)\s*(\d+[\.\d]*)?[:\.\s]*(.*?)(?=\n\n|\n(?=(?:theorem|definition|lemma|corollary|proposition|proof|example|remark))|\Z)", "lemma"),
    (r"(?i)^(corollary|cor\.?)\s*(\d+[\.\d]*)?[:\.\s]*(.*?)(?=\n\n|\n(?=(?:theorem|definition|lemma|corollary|proposition|proof|example|remark))|\Z)", "corollary"),
    (r"(?i)^(proposition|prop\.?)\s*(\d+[\.\d]*)?[:\.\s]*(.*?)(?=\n\n|\n(?=(?:theorem|definition|lemma|corollary|proposition|proof|example|remark))|\Z)", "proposition"),
    (r"(?i)^(axiom)\s*(\d+[\.\d]*)?[:\.\s]*(.*?)(?=\n\n|\n(?=(?:theorem|definition|lemma|corollary|proposition|proof|example|remark))|\Z)", "axiom"),
    (r"(?i)^(example)\s*(\d+[\.\d]*)?[:\.\s]*(.*?)(?=\n\n|\n(?=(?:theorem|definition|lemma|corollary|proposition|proof|example|remark|solution))|\Z)", "example"),
    (r"(?i)^(remark)\s*(\d+[\.\d]*)?[:\.\s]*(.*?)(?=\n\n|\n(?=(?:theorem|definition|lemma|corollary|proposition|proof|example|remark))|\Z)", "remark"),
]
...
def chunk_text_smart(text: str, source_book: str, chunk_size: int = 1000, overlap: int = 200) -> list[MathConcept]:
    """
...
def save_knowledge_base(
    ...
    meta = {
        ...
        "embedding_model": metadata.get("model_name", "unknown") if metadata else "unknown",
...
def main():
    parser = argparse.ArgumentParser(description="Extract math concepts and create knowledge base")
    parser.add_argument("--books-dir", type=str, default="books", help="Directory containing book text files")
    ...
    # Create embeddings
    embeddings = create_embeddings(all_concepts, model_name=args.model, batch_size=args.batch_size)
